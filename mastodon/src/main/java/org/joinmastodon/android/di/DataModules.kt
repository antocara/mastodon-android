...
val dataModules = module {
    ...
    single<OauthDataSource> {
        OauthRepository(AccountSessionManager.getInstance(), get(), get())
    }

    single { OauthTokenRequest(get(named(MASTODON_API))) }

    single { OwnAccountRequest(get(named(MASTODON_API))) }
